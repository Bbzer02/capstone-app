...
@section('title', 'CTU Danao HRMO - Job Management')
...
<div class="p-6">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.jobs.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg font-medium transition-colors duration-200 flex items-center space-x-2">
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $jobs->where('is_published', true)->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-200">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $jobs->where('is_published', false)->count() }}</p>
                </div>
            </div>
        </div>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Position Type</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $job->position_type }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $job->formattedSalary() }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            <div>
                                <div class="font-medium">{{ $job->getFormattedDeadlineAttribute() }}</div>
                                <div class="text-gray-500">{{ $job->application_deadline->diffForHumans() }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($job->is_published)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div class="flex items-center justify-center space-x-2">
                                ...
                                <form action="{{ route('admin.jobs.toggle-publish', $job) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" 
                                            class="inline-flex items-center px-3 py-2 border {{ $job->is_published ? 'border-amber-300 text-amber-700 bg-white hover:bg-amber-50' : 'border-green-300 text-green-700 bg-white hover:bg-green-50' }} shadow-sm text-sm leading-4 font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 {{ $job->is_published ? 'focus:ring-amber-500' : 'focus:ring-green-500' }} transition-colors duration-200">
                                        @if($job->is_published)
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- Delete Button -->
                                <form action="{{ route('admin.jobs.destroy', $job) }}" method="POST" class="inline delete-form">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" 
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <a href="{{ route('admin.jobs.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg font-medium transition-colors duration-200">
                                    Create New Job
                                </a>
                            </div>
                        </td>
